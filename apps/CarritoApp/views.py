...
def levantar_imagen_usuario(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # borro anterior si suben nueva
            if 'imagen_factura' in form.changed_data and factura.imagen_factura:
                factura.imagen_factura.delete(save=False)
            form.save()
            return redirect('CarritoApp:facturas_usuario')
    else:
...
def vista_resumen_factura(request, factura_id):
    ...
    # ====== Generar PDF ======
    pdf = canvas.Canvas(ruta_pdf)
    ancho, alto = pdf._pagesize  # para posicionar elementos en base al tamaño de hoja
    y = alto - 80  # margen superior

    # === Logo ===
    logo_path = os.path.join(settings.BASE_DIR, "static", "img", "atenealogo.jpeg")
    if os.path.exists(logo_path):
        try:
            pdf.drawImage(
                ImageReader(logo_path),
                x=40, y=alto - 120,  # posición
                width=80, height=80,  # tamaño del logo
                mask='auto'           # transparencia
            )
        except Exception as e:
            print(f"[WARN] No se pudo dibujar el logo: {e}")
    else:
        print(f"[WARN] No se encontró el logo en {logo_path}")

    # === Encabezado ===
    pdf.setFont("Helvetica-Bold", 16)
    pdf.drawString(150, alto - 60, "Atenea Indumentaria")
    pdf.setFont("Helvetica", 10)
    pdf.drawString(150, alto - 75, "www.ateneaquimili.com.ar")
    ...
    for p in productos:
        pdf.drawString(
            110, y,
            f"{p.get('nombre_producto')} x {p.get('cantidad_vendida')} - ${p.get('subtotal')}"
        )
        y -= 20
        if y < 100:
            pdf.showPage()
    ...
    pdf_url = request.build_absolute_uri(f"{media_url}/facturas/{nombre_pdf}?v={version}")
